...
function buildDefaultConfig() {
  ...
  return {
    ...
    hull: {
      ...
      // yawHeelSign: +-1, flips the heel->yaw coupling direction (aero.js
      // yawMomentHeel). Was "verified empirically against the 1.6
      // coupling-sign test (crew toward ama -> bear away)".
      //
      // AC-1 (2026-08-03): that calibration target is contradicted by the
      // owner's primary source. Kryteria_Akceptacji AC-1.1 says crew moving
      // TOWARD the ama points the bow UP -- the opposite of the rule this
      // sign was set to satisfy. It is the same pattern as AC-3 and the same
      // round: round 9 retired the lateral-crew steering channel and the
      // sheet-in-bears-away rule together, both as artefacts of an unphysical
      // lee-helm baseline. AC-3 has since been ruled the other way (adr/0014).
      //
      // FLIPPING THIS DOES NOT FIX AC-1, and that is the useful finding.
      // Measured at -1: AC-1.1 goes 1/6 -> 4/6 and AC-1.2 goes 6/6 -> 3/6 on
      // direction. It only trades one for the other, necessarily: the manual
      // says crew movement in EITHER direction points the bow up (AC-1.1 and
      // AC-1.2, by two different mechanisms it names explicitly), which is an
      // EVEN response in crew position, while the model's dominant crew->yaw
      // path runs through heel and is ODD in it. No choice of sign on an
      // antisymmetric term can produce a symmetric response.
      //
      // The model does contain both of the manual's mechanisms and they are
      // the right order of magnitude -- at a frozen state, crewPos 0->1 moves
      // amaDrag's yaw moment +31.8 N*m (the manual's AC-1.1 drag path) while
      // the heel term swings about +-27 N*m. The heel path simply wins on the
      // dynamics. Fixing this means making the drag path dominate on the
      // ama-loaded side and the righting-loss path on the unloaded side, not
      // turning a knob; left at +1 and reported rather than half-fixed. See
      // docs/findings-2026-08-02, AC-1 section.
      yawHeelSign: 1,
