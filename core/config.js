...
function buildDefaultConfig() {
  ...
  return {
    ...
    hull: {
      ...
      // coupling-sign test (crew toward ama -> bear away)" -- a target the
      // owner's primary source contradicts (docs/sources/, ch. III: "if crew
      // moves toward outrigger, the canoe turns to WINDWARD ... because the
      // ama sinks (creates drag) and rotates the canoe around").
      //
      // LEFT AT +1, AND THAT IS A REPORTED CONFLICT, NOT AN ENDORSEMENT.
      // This one sign governs two groups of criteria that the manual states
      // separately and that want opposite values. Measured across the whole
      // acceptance grid (harness/acceptance-manual.js):
      //
      //   criterion                              +1      0      -1
      //   AC-1.1 crew to ama -> points up       1/6    5/6    5/6
      //   AC-1.2 crew to hull -> bears away     0/6    4/6    4/6
      //   AC-4.1 main brail alone: no course    6/6    5/6    1/6
      //   AC-4.2a breaking brail -> bears away  6/6    1/6    1/6
      //   AC-4.2b ... and grows with the pull   4/6    1/6    0/6
      //
      // Every one of those five is stated explicitly in the manual, so no
      // value of this sign satisfies the source. Zeroing the coupling does not
      // resolve it either: it fixes the crew group and leaves the brail group
      // just as wrong, which shows the brail's bear-away is currently RIDING
      // this term rather than coming from the mechanism the manual names for
      // it (spilling the leech moves the CE forward -> lee helm).
      //
      // So the defect is one of relative MAGNITUDE, not of sign: the heel->yaw
      // coupling is strong enough to dominate both groups and force them to
      // share a direction, while the two mechanisms the manual actually names
      // -- the ama's drag for crew position, the CE shift for the brail -- are
      // present but too weak to govern their own criteria. Fixing it means
      // rebalancing those three against each other, which is a multi-parameter
      // change with a polar diff behind it and an owner's decision in front of
      // it, not a knob to turn here. +1 is kept because it is what the current
      // suite and polar were measured against.
      yawHeelSign: 1,
