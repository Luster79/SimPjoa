...
const boatVariantSelect = document.getElementById('boatVariant');

// Switching the named boat is NOT a field edit and cannot go through
// collectBoatPatch: every value in BOAT_FIELDS is derived from the variant's
// parameter file, so sending the current field values along would overwrite
// the boat being switched to with the one being switched from. Send only the
// variant, then rebuild the panel from whatever it produced.
function applyBoatVariant(variant) {
  let validated;
  try {
    validated = createConfig({ boat: variant });
  } catch (err) {
...
boatVariantSelect.addEventListener('change', () => applyBoatVariant(boatVariantSelect.value));

function buildBoatPanel() {
  buildBoatFieldRows();
  refreshBoatPresetSelect();
  boatVariantSelect.value = dims.boat ?? 'default';
}
...
function collectBoatPatch() {
  // Carries the selected variant, so Apply re-derives from the boat currently
  // on screen. Without it createConfig() would rebuild from the DEFAULT boat
  // and overlay these fields on top -- silently reverting everything the
  // variant sets that has no field row (residuaryPeakCr, every inertia).
  const patch = { boat: boatVariantSelect.value };
...
document.getElementById('btnResetBoatDefaults').addEventListener('click', () => {
  // Defaults OF THE SELECTED BOAT -- resetting on the old boat must not
  // silently hand back the default one.
  dims = createConfig({ boat: boatVariantSelect.value });
  updateBrailZoneUI();
  updateSheetRangeUI();
  renderCrewPad();
  buildBoatPanel();
