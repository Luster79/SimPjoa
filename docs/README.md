...
| `work-order-2026-08-10-blok-B.md` | **Open, not started.** Derived from what block A actually measured, not from the proposal that preceded it -- one item of that proposal (widen the CE-CLR lever) is **withdrawn outright**, because M2 measured the tack null in range with better than 2x margin at all nine beat points. `N1` is the measurement debt: re-score L4/L5 with the union metric, since their ADRs' "Measured" sections were written against a metric losing 8-17 points. `N2` closes the four remaining gaps (all TWS6, all where the coarse sheet grid fits worst). `N3` adds a second, continuously-trimmed predicate beside `holdsCourse` -- L3 (the manual describes active trimming, and prefaces ch. III with "use with moderation") and M2 (the beat failure is exactly what a crew corrects reflexively) point at the same question, and **which predicate is the criterion is the owner's decision**, not a technical change. `N4` is the last piece of the oar-free shunt: accelerating from a dead stop. `N5` is the close-hauled drive deficit vs Di Piazza (`S4b`), the realism half. |
