...
| `work-order-2026-08-02-steering-and-sources.md` | Open: **S7** (R15 done; the rest outstanding), **S8** (vertical balance) -- both code-hygiene/scope items, unrelated to steering itself. S3 was implemented and withdrawn (ADR 0013); everything else is done. Sailing without the oar reached 3/6 (ADR 0017); the Munk double-count hypothesis was audited and rejected (ADR 0018). What used to block further progress here on Flay's yaw-moment data is resolved two ways, not one: that data does not exist in the source at all (confirmed by reading the full paper, ADR 0032), and D1 found a geometric route around the gap instead (same ADR). |
