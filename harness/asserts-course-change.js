...
// without the oar in the first place. The two checks below do, with
...
// transition. (A third, O8/O9's long staged walk, moved to
// harness/report-long-walks.js -- W4, Archive/work-order-2026-08-15-pelny-wiatr.md.)
//
...
export function check_course_change(config, check, slow) {
  ...
  for (const end of [1, -1]) {
    ...
    check(`K3: pointing up (TWA90 -> TWA70, TWS6, end=${end}) reached under trim alone with the oar shipped throughout, and holds`,
      ...
      `startHoldConfirmed=${pointUp.fromConfirmed} reached TWA${pointUp.twaReached.toFixed(1)} (target 70+-10, excursion from start ${pointUp.excursion.toFixed(1)}deg) converged=${pointUp.converged} restoring=${pointUp.restoring} speedRatio=${(pointUp.speedRatio * 100).toFixed(0)}% capsized=${pointUp.capsized} -- PROMOTED out of xfail 2026-08-11 by O1 (reached 79.6/79.8deg, margin 0.2-0.4deg against the 70+-10 band), DEMOTED back 2026-08-15 by ADR 0047 (W1, Archive/work-order-2026-08-15-pelny-wiatr.md): windage's new yaw-moment lever (core/aero.js) is driven by apparent wind, which is LARGEST close-hauled/reaching -- exactly this pair's operating point -- so the same term that is negligible at the deep courses it was added for (1-3 N*m, ADR 0047) is large enough here to move the settled trim from 79.6/79.8 to 85.2/85.3, outside the ±10deg band. A real trade the new physics makes, not a search regression: the margin O1 found was already narrow (0.2-0.4deg) and one wind`,
      'STEERING');
  }

  // O8/O9 (long course changes, walked through the 10deg grid) moved out of
  // this CI gate to harness/report-long-walks.js -- W4 (docs/work-order-
  // 2026-08-15-pelny-wiatr.md), owner decision 2026-08-15: the two checks
  // added ~50-60min to a full run by requiring an exhausted search at every
  // waypoint that has no holding trim, and nobody had decided what a
  // regression there should do to the build. They are measured and reported
  // there now, same status as coverage-no-oar.js and coverage-obtain-course.js.
